...
fn test_tangent_symmetric_at_zero() {
    ...
    let k = assembler.assemble_tangent::<LinearBMatrix>(&mesh, &mat, &u, &SimulationContext::isotropic_static(mesh.elements.len()));
...
fn test_internal_forces_zero_at_rest() {
    ...
    let assembler = Assembler::new(&mesh);
    let u = DVector::zeros(3 * mesh.nodes.len());
    let f_int = assembler.assemble_internal_forces(&mesh, &mat, &u, &mut SimulationContext::isotropic_static(mesh.elements.len()));
...
fn test_internal_forces_consistent_with_tangent() {
    ...
    let k = assembler.assemble_tangent::<LinearBMatrix>(&mesh, &mat, &u_zero, &mut SimulationContext::isotropic_static(mesh.elements.len()));

    let mut u_small = DVector::zeros(n);
    u_small[3] = 1e-6;
    u_small[4] = 1e-6;

    let f_int = assembler.assemble_internal_forces(&mesh, &mat, &u_small, &mut SimulationContext::isotropic_static(mesh.elements.len()));
    let f_lin = &k * &u_small;
...
fn test_assemble_method_comparaison() {
    ...
    let k_dense  = assembler.assemble_tangent::<LinearBMatrix>(&mesh, &mat, &u_zero, &SimulationContext::isotropic_static(mesh.elements.len()));
    let k_sparse = assembler.assemble_tangent_sparse::<LinearBMatrix>(&mesh, &mat, &u_zero, &SimulationContext::isotropic_static(mesh.elements.len()));
...
fn test_assemble_parrallel_method_comparaison() {
    ...
    let assembler = Assembler::new(&mesh);
    let u_zero = DVector::zeros(3 * mesh.nodes.len());
    let k_par = assembler.assemble_tangent_sparse_parallel::<LinearBMatrix>(&mesh, &mat, &u_zero, &SimulationContext::isotropic_static(mesh.elements.len()));
    let k_seq = assembler.assemble_tangent_sparse::<LinearBMatrix>(&mesh, &mat, &u_zero, &SimulationContext::isotropic_static(mesh.elements.len()));
...
fn test_assemble_parallel_speedup() {
    ...
    let _     = assembler.assemble_tangent_sparse::<LinearBMatrix>(&mesh, &mat, &u_zero, &SimulationContext::isotropic_static(mesh.elements.len()));
    let t_seq = t0.elapsed();

    let t0    = std::time::Instant::now();
    let _     = assembler.assemble_tangent_sparse_parallel::<LinearBMatrix>(&mesh, &mat, &u_zero, &SimulationContext::isotropic_static(mesh.elements.len()));
