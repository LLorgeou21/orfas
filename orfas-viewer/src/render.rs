...
pub fn depth_sorted_nodes(mesh: &Mesh, camera: &Camera) -> Vec<usize> {
    let forward = camera.view_direction();
    let mut indices: Vec<usize> = (0..mesh.nodes.len()).collect();
    indices.sort_by(|&a, &b| {
        let pa = mesh.nodes[a].position.cast::<f32>() - camera.target;
        let pb = mesh.nodes[b].position.cast::<f32>() - camera.target;
        // Depth = projection onto view direction; larger = further
